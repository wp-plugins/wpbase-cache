...
class DBCacheReloaded {
	...
	// Enable cache
	function dbcr_enable( $echo = false ) {
		$status = true;
	...
	// Disable cache
	function dbcr_disable( $echo = false ) {
	...
	function options_page($options) {
		...
		if ( isset( $options['clear'] ) ) {
			...
			//check_admin_referer( 'db-cache-reloaded-update-options' );
			$saveconfig = $this->config = $options;
...
// html for options page was there
	}
